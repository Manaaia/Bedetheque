...
require_once('Accueil/Models/connexionBDD.class.php');
...
class User {
    ...
    private function setIdUser($id_user) {
        if (preg_match("/[0-9]{9}/",$this->id_user)) {
    ...
    private function setNomUser($nom_user) {
        if (preg_match("[a-zA-Z-]",$this->nom_user)) {
    ...
    private function setPrenomUser($prenom_user) {
        if (preg_match("[a-zA-Z-]",$this->prenom_user)) {
    ...
    private function setMdp($mdp) {
        if (preg_match("^(?=\P{Ll}*\p{Ll})(?=\P{Lu}*\p{Lu})(?=\P{N}*\p{N})(?=[\p{L}\p{N}]*[^\p{L}\p{N}])[\s\S]{8,}$",$this->mdp)) {
            $this->mdp = $mdp;
        } else {
    ...
    public function setIdRole($id_role) {
        ...
        $rolelist = $result->fetchAll(PDO::FETCH_DEFAULT);

        $result->closeCursor();
        connexionBDD::disconnect();

        if (in_array($this->id_role,$rolelist)) {
    ...
    public function __toString() {
        $message = "Infos utilisateur :<br/>Id utilisateur : ".$this->id_user."<br/>Nom : ".$this->nom_user."<br/>Prénom : ".$this->prenom_user
        ."Adresse : ".$this->adresse_1_user.", ".$this->adresse_2_user.", ".$this->cp_user.", ".$this->ville_user."<br/>Date de cotisation : ".$this->date_cotisation
        ."Rôle : ".$this->id_role;
